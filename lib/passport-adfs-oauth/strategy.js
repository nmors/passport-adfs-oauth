...
function Strategy(options, verify) {
  ...
  this._authURL = this._loginUrl + this._authRelativeURL;
  this._tokenURL = this._loginUrl + this._tokenRelativeURL;
...
/**
 * Authenticate request by delegating to the Adfs OAuth 2.0 provider.
 *
 * @param {Object} req
 * @api protected
 */
Strategy.prototype.authenticate = function(req, options) {
  ...
  this._tokenURL = this._loginUrl + this._tokenRelativeURL;
	...
	if (!this._clientId) throw new Error('AdfsOAuthStrategy requires a clientId.');
	// if (!this._clientSecret) throw new Error('AdfsOAuthStrategy requires a clientSecret.');
	//if (!this._tenantId) throw new Error('AdfsOAuthStrategy requires a tenant id.');
	if (!this._resource) throw new Error('AdfsOAuthStrategy requires a token URL.');

  // check if there is a app token present
  if (typeof this._refreshToken != 'undefined') {
    console.error('refresh area--------->')
    ...
    this._oauth2 = new OAuth2(this._clientId, '', '', this._authURL, this._tokenURL, '', this.proxy);
    ...
    params['response_type'] = 'code';
		if (this.redirectURL) {
      params['redirect_uri'] = this.redirectURL;// + "?" + this.authorizationParamsForUrl(options);
    }
...
/**
 * Retrieve user profile from Adfs.
 *
...
Strategy.prototype.authorizationParams = function(options) {
  ...
  for (var attr in options) {
    //if (attr != 'clientId' && attr != 'clientSecret' && attr != 'tenantId') {
    // if (attr != 'clientId' && attr != 'clientSecret') {
    if (attr != 'clientId') {
